...
fn main() {
    ...
    if num_cpus == 0 {
        num_cpus = num_cpus::get();  // Use all logical cores (including hyperthreads)
    }
    ...
    // Hardcoded gcode filename (ignore line from Input_file.txt)
    let gcodefile = "inputfiles/wall.gcode";
    ...
    if model_type == 0 {
        ...
        write_buffer.flush().expect("failed to flush activation times");
        drop(write_buffer);
